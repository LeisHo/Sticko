...
- Procedural only — every stick-figure motion (walking, idling, falling, flying, fleeing, getting back up) must be driven by the physics engine + small steering/behavior rules, never a hand-authored keyframe animation. If a behavior needs scripting, script the *rule* ("push toward this target"), not the motion itself.
